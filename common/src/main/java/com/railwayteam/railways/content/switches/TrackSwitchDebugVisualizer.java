            ...
            for (TrackNodeLocation tnl : graph.getNodes()) {
                ...
                if (connectionCount > 2 && connectionCount <= 4) {
                    ...
                    for (Map.Entry<TrackNode, TrackEdge> entry : connections.entrySet()) {
                        Vec3 off = entry.getKey().getLocation().getLocation()
                                .subtract(basePos)
                                .normalize();
                        averageOffset = averageOffset.add(off);
                        offsets[i] = off;
                        i++;
                    }
                    ...
                    for (Vec3 off : offsets) {
                        double distance = averageOffset.distanceToSqr(off);
                    ...
                    Outliner.getInstance().chaseAABB(node, AABB.ofSize(tnl.getLocation()
